...
function roleFill(role: MuscleRole | undefined, level: number) {
  if (level <= 0) return levelFill(0);
  if (role === "secondary") return level >= 3 ? "rgba(184, 151, 255, 0.78)" : "rgba(145, 108, 219, 0.58)";
  if (role === "tertiary") return level >= 3 ? "rgba(215, 198, 255, 0.72)" : "rgba(167, 156, 196, 0.52)";
  return levelFill(level);
}

function MuscleShape({ d, k, levels, state }: { d: string; k: MuscleKey; levels?: MuscleLevels; state?: MuscleState }) {
...
export function BodyFront({ levels, state, gender = "male" }: { levels?: MuscleLevels; state?: MuscleState; gender?: BodyGender }) {
  return (
    <svg viewBox="0 0 220 490" className="h-full w-full" aria-label={`Corpo ${gender === "female" ? "feminino" : "masculino"}, frente`}>
      <SharedDefs />
      ...
      <MuscleShape state={state} k="shoulders" levels={levels} d="M54 92 Q42 108 47 136 Q57 144 73 140 Q78 119 73 100 Q65 90 54 92 Z" />
      <MuscleShape state={state} k="shoulders" levels={levels} d="M166 92 Q178 108 173 136 Q163 144 147 140 Q142 119 147 100 Q155 90 166 92 Z" />
      <MuscleShape state={state} k="chest" levels={levels} d="M73 98 Q93 86 108 92 Q111 114 108 143 Q91 148 68 136 Q64 116 73 98 Z" />
      <MuscleShape state={state} k="chest" levels={levels} d="M147 98 Q127 86 112 92 Q109 114 112 143 Q129 148 152 136 Q156 116 147 98 Z" />
      <MuscleShape state={state} k="abs" levels={levels} d="M96 145 Q110 141 124 145 L122 172 Q110 176 98 172 Z" />
      <MuscleShape state={state} k="abs" levels={levels} d="M96 176 Q110 171 124 176 L122 203 Q110 206 98 203 Z" />
      <MuscleShape state={state} k="abs" levels={levels} d="M96 207 Q110 204 124 207 L121 235 Q110 239 99 235 Z" />
      <MuscleShape state={state} k="obliques" levels={levels} d="M78 149 Q90 164 89 232 Q78 235 71 224 Q69 181 78 149 Z" />
      <MuscleShape state={state} k="obliques" levels={levels} d="M142 149 Q130 164 131 232 Q142 235 149 224 Q151 181 142 149 Z" />
      <MuscleShape state={state} k="biceps" levels={levels} d="M45 145 Q36 171 41 197 Q47 203 56 198 Q64 173 60 150 Q52 142 45 145 Z" />
      <MuscleShape state={state} k="biceps" levels={levels} d="M175 145 Q184 171 179 197 Q173 203 164 198 Q156 173 160 150 Q168 142 175 145 Z" />
      <MuscleShape state={state} k="forearms" levels={levels} d="M45 203 Q39 242 49 299 Q56 303 63 296 Q64 250 56 201 Z" />
      <MuscleShape state={state} k="forearms" levels={levels} d="M175 203 Q181 242 171 299 Q164 303 157 296 Q156 250 164 201 Z" />
      <MuscleShape state={state} k="quads" levels={levels} d="M79 286 Q92 321 93 438 Q80 443 69 433 Q66 351 79 286 Z" />
      <MuscleShape state={state} k="quads" levels={levels} d="M141 286 Q128 321 127 438 Q140 443 151 433 Q154 351 141 286 Z" />
      <MuscleShape state={state} k="calves" levels={levels} d="M79 377 Q71 407 75 442 Q81 447 89 442 Q94 408 92 380 Z" />
      <MuscleShape state={state} k="calves" levels={levels} d="M141 377 Q149 407 145 442 Q139 447 131 442 Q126 408 128 380 Z" />
...
export function BodyBack({ levels, state, gender = "male" }: { levels?: MuscleLevels; state?: MuscleState; gender?: BodyGender }) {
  return (
    <svg viewBox="0 0 220 490" className="h-full w-full" aria-label={`Corpo ${gender === "female" ? "feminino" : "masculino"}, costas`}>
      <SharedDefs />
      <rect x="0" y="0" width="220" height="490" fill="transparent" />
      <ellipse cx="110" cy="220" rx="90" ry="190" fill="url(#eforge-core-glow)" opacity="0.55" />
      <BodyShellBack />

      <MuscleShape state={state} k="traps" levels={levels} d="M92 67 Q110 58 128 67 Q134 98 126 143 L110 166 L94 143 Q86 98 92 67 Z" />
      <MuscleShape state={state} k="rear_delts" levels={levels} d="M54 93 Q44 106 47 134 Q58 142 74 139 Q78 121 73 102 Q64 91 54 93 Z" />
      <MuscleShape state={state} k="rear_delts" levels={levels} d="M166 93 Q176 106 173 134 Q162 142 146 139 Q142 121 147 102 Q156 91 166 93 Z" />
      <MuscleShape state={state} k="lats" levels={levels} d="M73 106 Q86 121 95 212 Q80 220 65 205 Q60 154 73 106 Z" />
      <MuscleShape state={state} k="lats" levels={levels} d="M147 106 Q134 121 125 212 Q140 220 155 205 Q160 154 147 106 Z" />
      <MuscleShape state={state} k="lower_back" levels={levels} d="M98 188 Q110 183 122 188 L121 237 Q110 243 99 237 Z" />
      <MuscleShape state={state} k="triceps" levels={levels} d="M45 145 Q36 171 41 197 Q47 203 56 198 Q64 173 60 150 Q52 142 45 145 Z" />
      <MuscleShape state={state} k="triceps" levels={levels} d="M175 145 Q184 171 179 197 Q173 203 164 198 Q156 173 160 150 Q168 142 175 145 Z" />
      <MuscleShape state={state} k="forearms" levels={levels} d="M45 203 Q39 242 49 299 Q56 303 63 296 Q64 250 56 201 Z" />
      <MuscleShape state={state} k="forearms" levels={levels} d="M175 203 Q181 242 171 299 Q164 303 157 296 Q156 250 164 201 Z" />
      <MuscleShape state={state} k="glutes" levels={levels} d="M78 241 Q98 247 105 280 Q89 294 70 286 Q63 261 78 241 Z" />
      <MuscleShape state={state} k="glutes" levels={levels} d="M142 241 Q122 247 115 280 Q131 294 150 286 Q157 261 142 241 Z" />
      <MuscleShape state={state} k="hamstrings" levels={levels} d="M80 289 Q91 337 92 435 Q82 441 72 435 Q69 337 80 289 Z" />
      <MuscleShape state={state} k="hamstrings" levels={levels} d="M140 289 Q129 337 128 435 Q138 441 148 435 Q151 337 140 289 Z" />
      <MuscleShape state={state} k="calves" levels={levels} d="M79 381 Q70 410 75 444 Q82 449 90 444 Q95 409 92 382 Z" />
      <MuscleShape state={state} k="calves" levels={levels} d="M141 381 Q150 410 145 444 Q138 449 130 444 Q125 409 128 382 Z" />
